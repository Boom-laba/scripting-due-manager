...
export function advanceManualItem(
  ...
  if (recurrence.fromCompletion) {
    const completedDate = localDateKey(now)
    const parts = parseDateKey(completedDate)!
    recurrence = {
      ...recurrence,
      anchorDay: parts.day,
      anchorMonth: parts.month,
      useMonthEnd: recurrence.useMonthEnd,
    }
  }
  let dueDate = nextOccurrence(recurrence.fromCompletion ? localDateKey(now) : item.dueDate, recurrence)
  ...
  return { ...item, dueDate, recurrence, updatedAt: now.getTime() }
}
...
export function dueStatus(
  item: DueStatusInput,
  now = new Date(),
): { days: number; overdue: boolean; needsAction: boolean; label: string; color: Color } {
