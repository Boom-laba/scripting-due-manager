import { EmailView } from "./src/email_view"
import { CostOverviewView } from "./src/cost_view"
import { syncEmail } from "./src/email"
import {
  Button,
  DatePicker,
  Device,
...
const REMINDER_HORIZON_OPTIONS = [
  { days: 30, label: "未来 30 天" },
  { days: 90, label: "未来 3 个月" },
  { days: 365, label: "未来 1 年" },
  { days: 730, label: "未来 2 年" },
  { days: 3650, label: "未来 10 年" },
] as const
...
function DueManagerApp() {
  ...
  const setReminderHorizon = async (reminderHorizonDays: number) => {
    ++reminderRequests.generation
    try {
      const next = updateSettings({ reminderHorizonDays })
      refreshState(next)
      ...
      setReminderStatus(status => ({ ...status, loading: true }))
      const result = await loadReminderItems(next.settings.reminderHorizonDays, next.settings.reminderCalendarIDs)
      setReminderStatus({ loading: false, count: result.items.length, fetchedAt: result.fetchedAt,
        live: result.live, fromCache: result.fromCache, error: result.error })
      await refreshWidgetsWithWarning("读取范围已保存")
    } catch (error) {
      setReminderStatus(status => ({ ...status, loading: false, error: String(error) }))
      await Dialog.alert({ title: "读取范围保存失败", message: String(error) })
    }
  }
    ...
    >
      ...
      >
        ...
        {state.settings.includeReminders
          ? <Picker
            title="读取范围"
            value={state.settings.reminderHorizonDays}
            disabled={reminderStatus.loading}
            pickerStyle="menu"
            onChanged={(value: number) => { void setReminderHorizon(value) }}
          >
            {REMINDER_HORIZON_OPTIONS.map(option => (
              <Text key={option.days} tag={option.days}>{option.label}</Text>
            ))}
          </Picker>
          : null}
        {state.settings.includeReminders
      ...
      <Section header={<Text>显示与组件</Text>}>
        <NavigationLink destination={<CostOverviewView items={state.items} />}>
          <Label title="费用概览" systemImage="chart.bar.xaxis" />
        </NavigationLink>
        ...
        <NavigationLink destination={<EmailView />}><Label title="云邮件提醒" systemImage="envelope" /></NavigationLink>
        <Toggle
          title="在组件显示金额"
...
function ItemEditor({
  ...
  const [recurrenceMode, setRecurrenceMode] = useState<"scheduled" | "completion">(
    item.recurrence?.fromCompletion ? "completion" : "scheduled",
  )
    ...
    if (recurrenceUnit !== "none") {
      ...
      recurrence = preserveAnchor
        ? {
          ...
          useMonthEnd,
          leapDayPolicy,
          fromCompletion: recurrenceMode === "completion",
        }
        : createRecurrenceRule(
          recurrenceUnit,
          recurrenceInterval!,
  ...
  const duplicate = async () => {
    ...
    const source = buildItem()
    if (!source) return
    const draft = createDraftItem()
    const suffix = " 副本"
    const copy: ManualDueItem = {
      ...source,
      id: draft.id,
      title: `${source.title.slice(0, 120 - suffix.length)}${suffix}`,
      enabled: true,
      createdAt: draft.createdAt,
      updatedAt: draft.updatedAt,
    }
    try {
      const nextState = upsertItem(copy)
      onChanged(nextState)
      const warning = await refreshAfterDataChange()
      if (warning) await Dialog.alert({ title: "副本已创建", message: warning })
      dismiss()
    } catch (error) {
      await Dialog.alert({ title: "创建副本失败", message: String(error) })
    }
  }
  ...
  >
    ...
    <Section
      header={<Text>重复</Text>}
      footer={
        <Text>{`间隔可输入 ${MIN_RECURRENCE_INTERVAL}–${MAX_RECURRENCE_INTERVAL} 的正整数。“按原计划”保持日历锚点；“按完成日期”会在完成本期后再计算下一期。提前提醒可输入 ${MIN_REMIND_BEFORE_DAYS}–${MAX_REMIND_BEFORE_DAYS} 天，0 表示不提前。`}</Text>
      }
    >
      <Picker
        title="周期"
      ...
      {recurrenceUnit !== "none"
        ? <Picker
          ...
          onChanged={setRecurrenceMode as any}
          pickerStyle="menu"
        >
          <Text tag="scheduled">按原计划日期</Text>
          <Text tag="completion">按完成日期滚动</Text>
        </Picker>
        : null}
    ...
    {!isNew
      ? <Section header={<Text>事项管理</Text>}>
        <Button
          title="复制为新事项"
          systemImage="doc.on.doc"
          action={() => { void runEditorAction(duplicate) }}
        />
        <Button
          title="删除事项"
          systemImage="trash"
...
async function run() {
  ...
  void syncEmail().catch(() => undefined)
